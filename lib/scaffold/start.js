...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for ufod is necessary for this upgrade with the "reindex=1" ufocoin.conf option. \n' +
                 'There are changes necessary in both ufo.conf and ufocore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/ufod')
            }
          }
        }
      };
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ufocore-node.json');
